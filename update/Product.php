...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_product'])) {
        ...
        if ($_FILES['product_image']['size'] > 3 * 1024 * 1024) {
            ...
            if ($imageDimensions[0] < 225 || $imageDimensions[1] < 224 || $imageDimensions[0] > 300 || $imageDimensions[1] > 300) {
                ...
                if (move_uploaded_file($_FILES['product_image']['tmp_name'], $target_file)) {
                    $sql = "INSERT INTO product (`Merchandise ID`, name, price, small_stock, medium_stock, large_stock, xl_stock, xxl_stock, category, image) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    $stmt = $conn->prepare($sql);
                    $stmt->bind_param("sssssssss", $product_name, $product_price, $small_stock, $medium_stock, $large_stock, $xl_stock, $xxl_stock, $product_category, $target_file);
                    $stmt->execute();
        ...
        $small_stock = $_POST['update_p_small_stock'];
        $medium_stock = $_POST['update_p_medium_stock'];
        $large_stock = $_POST['update_p_large_stock'];
        $xl_stock = $_POST['update_p_xl_stock'];
        $xxl_stock = $_POST['update_p_xxl_stock'];
        ...
        if ($_FILES['update_p_image']['size'] > 3 * 1024 * 1024) {
            ...
            if ($imageDimensions[0] < 225 || $imageDimensions[1] < 224 || $imageDimensions[0] > 300 || $imageDimensions[1] > 300) {
                ...
                if (move_uploaded_file($_FILES['update_p_image']['tmp_name'], $target_file)) {
                    $sql = "UPDATE product SET name=?, price=?, small_stock=?, medium_stock=?, large_stock=?, xl_stock=?, xxl_stock=?, category=?, image=? WHERE `Merchandise ID`=?";
                    $stmt = $conn->prepare($sql);
                    $stmt->bind_param("ssiiiiissi", $product_name, $product_price, $small_stock, $medium_stock, $large_stock, $xl_stock, $xxl_stock, $product_category, $target_file, $product_id);
                    $stmt->execute();
...
if ($is_admin) {
    $sql = "SELECT `Merchandise ID`, name, price, small_stock, medium_stock, large_stock, xl_stock, xxl_stock, category, image FROM product";
    $result = $conn->query($sql);
} else {
    $sql = "SELECT name, price, small_stock, medium_stock, large_stock, xl_stock, xxl_stock, category, image FROM product";
    $result = $conn->query($sql);
}
?>
...
<body>
        ...
        <main class="main-content">
            ...
            <div class="add-product-form" id="addProductForm">
                ...
                <form action="Product.php" method="post" enctype="multipart/form-data" id="addProductForm" onsubmit="return validateImage()">
                    ...
                    <select name="product_category" class="box" id="productCategory" required>
                ...
                <section class="update-form-container hidden" id="updateFormContainer">
                    ...
                    <div class="update-product-form">
                        ...
                        <form action="Product.php" method="post" enctype="multipart/form-data" onsubmit="return validateImage()">
                            ...
                            <input type="number" min="0" class="box" required name="update_p_small_stock" value="<?php echo htmlspecialchars($fetch_edit['small_stock']); ?>" placeholder="Enter the small stock">
                            <input type="number" min="0" class="box" required name="update_p_medium_stock" value="<?php echo htmlspecialchars($fetch_edit['medium_stock']); ?>" placeholder="Enter the medium stock">
                            <input type="number" min="0" class="box" required name="update_p_large_stock" value="<?php echo htmlspecialchars($fetch_edit['large_stock']); ?>" placeholder="Enter the large stock">
                            <input type="number" min="0" class="box" required name="update_p_xl_stock" value="<?php echo htmlspecialchars($fetch_edit['xl_stock']); ?>" placeholder="Enter the XL stock">
                            <input type="number" min="0" class="box" required name="update_p_xxl_stock" value="<?php echo htmlspecialchars($fetch_edit['xxl_stock']); ?>" placeholder="Enter the XXL stock">
                ...
                <div class="product-list">
                    <?php while($row = $result->fetch_assoc()): ?>
                        <div class="product-card">
                            ...
                            $total_stock = $row['small_stock'] + $row['medium_stock'] + $row['large_stock'] + $row['xl_stock'] + $row['xxl_stock'];
                            ?>
                            ...
                            <?php else: ?>
                                <button class="btn" onclick="showReservedForm()"> Reserved </button>
                            <?php endif; ?>
                        </div>
                    <?php endwhile; ?>
                </div>
    ...
    <!-- Reserved Form Modal -->
    <div id="reservedFormModal" class="modal">
        <div class="modal-content">
            <span class="close" onclick="closeReservedForm()">&times;</span>
            <h2>Reserved Form</h2>
    ...
    <script>
        ...
        // logic in disabling sizes text fields
        function handleCategoryChange() {
            console.log('Change event triggered');
            var sizeFields = document.querySelectorAll('.size-field');
            console.log('Size fields:', sizeFields);
            var disableSizes = document.getElementById('productCategory').value === 'Accessories';
            console.log('Disable sizes:', disableSizes);
            sizeFields.forEach(function(field) {
                console.log('Processing field:', field);
                field.disabled = disableSizes;
                if (disableSizes) {
                field.value = '';
                }
            });
        ...
        window.onclick = function(event) {
            const modal = document.getElementById('reservedFormModal');
